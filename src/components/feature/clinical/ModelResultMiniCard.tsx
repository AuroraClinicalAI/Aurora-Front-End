...
const symptoms = (clasificacion.ml_sintomas_identificados as Record<string, number>) || {};
const dsm5 = (clasificacion.ml_dsm5_evaluacion as Record<string, unknown>) || {};
    ...
    <CardContent className="p-0">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          {Object.entries(symptoms).map(([label, val], i) => (
            <div key={i} className="space-y-1.5">
              <div className="flex justify-between text-[9px] font-bold text-zinc-900">
                <span>{label}</span>
                <span>{Number((val * 100).toFixed(0))}%</span>
              </div>
              <div className="h-1 w-full bg-zinc-100 rounded-full overflow-hidden">
                <div className="h-full bg-indigo-400 rounded-full" style={{ width: `${Number((val * 100).toFixed(0))}%` }} />
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="mt-8 pt-6 border-t border-zinc-50 space-y-3">
        <div className="p-4 rounded-xl border border-zinc-100 flex flex-col items-center text-center mb-2">
          ...
          <span className="text-[10px] text-zinc-900 font-bold">{dsm5.meets_criteria ? 'Cumple Criterios Mayores de Depresión' : 'No cumple Criterios de manera concluyente'}</span>
        </div>
